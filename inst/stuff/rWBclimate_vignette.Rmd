...
```{r loadlib, echo=FALSE,warning=FALSE,message=FALSE,results='hide',cache=TRUE}
library(ggplot2)
library(rWBclimate)
```

```{r getmodeldata,message=FALSE,cache=TRUE}
...
```{r comparing quantiles,cache=TRUE}
...
```{r historicalyear, message=FALSE,warning=FALSE,cache=TRUE}
...
```{r map_plot,fig.width=5.5, fig.height=7,warning=FALSE,message=FALSE,cache=TRUE}
...
```{r climatemap,fig.width=5.5, fig.height=7,message=FALSE,cache=TRUE}
